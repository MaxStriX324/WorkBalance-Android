...
- баланс с учётом сегодняшнего дня и время выхода «в ноль»;
...
- локальное хранение без аккаунта, аналитики и сервера.
...
Приложение не запрашивает доступ к интернету и не передаёт данные разработчику. Все отметки хранятся в локальной базе Room. Экспорт создаётся только по действию пользователя через системный выбор файла.
...
- Room;
- Coroutines и Flow;
- AlarmManager для напоминания об обеде;
...
- [x] Статистика месяца;
- [ ] Напоминание о забытом входе или выходе;
...
Автор: [maxstrix324](https://github.com/maxstrix324)
